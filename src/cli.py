...
def _format_validation_report(report: dict[str, Any]) -> str:
    ...
    if isinstance(csr_artifact, dict):
        parts.extend(_matrix_view_text_parts(csr_artifact.get("matrixViews")))
...
def _performance_rollup_text_parts(summary: dict[str, Any]) -> list[str]:
    parts = []
    for key in (
        "performanceReports",
...
def _format_report_bundle_result(
    ...
    *,
    ...
    if isinstance(triage_summary, dict):
        ...
        parts.extend(_performance_rollup_text_parts(triage_summary))
    return " ".join(parts)
...
def _format_graph_diff_tree_report(report: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        f"leftNodes={int(summary.get('leftNodes', 0) or 0)}",
        f"rightNodes={int(summary.get('rightNodes', 0) or 0)}",
        f"nodeDelta={int(summary.get('nodeDelta', 0) or 0)}",
        f"addedNodes={int(summary.get('addedNodes', 0) or 0)}",
        f"removedNodes={int(summary.get('removedNodes', 0) or 0)}",
        f"metadataChangedNodes={int(summary.get('metadataChangedNodes', 0) or 0)}",
        f"nodeChurn={int(summary.get('nodeChurn', 0) or 0)}",
        f"leftEdges={int(summary.get('leftEdges', 0) or 0)}",
        f"rightEdges={int(summary.get('rightEdges', 0) or 0)}",
        f"edgeDelta={int(summary.get('edgeDelta', 0) or 0)}",
        f"addedEdges={int(summary.get('addedEdges', 0) or 0)}",
        f"removedEdges={int(summary.get('removedEdges', 0) or 0)}",
        f"edgeChurn={int(summary.get('edgeChurn', 0) or 0)}",
        f"classifiedChanges={int(summary.get('classifiedChanges', 0) or 0)}",
    ]
    for key in (
        "upgradeChanges",
...
def _format_csr_artifact_manifest(
    ...
    *,
    ...
    parts = [
        ...
        *_matrix_view_text_parts(manifest.get("matrixViews")),
...
def _matrix_view_text_parts(matrix_views: Any) -> list[str]:
