"""Generate a self-checking browser smoke page for graph-diff filters."""
...
SMOKE_OUTPUT_PATH = Path("/tmp/edgp-graph-diff-filters-smoke.html")
SMOKE_INPUT_PATH = REPO_ROOT / "tests" / "fixtures" / "graph-diff.json"


def render_graph_diff_filter_smoke_page() -> str:
...
def write_graph_diff_filter_smoke_page(
    output_path: Path = SMOKE_OUTPUT_PATH,
) -> Path:
    output_path.write_text(render_graph_diff_filter_smoke_page(), encoding="utf-8")
    return output_path
...
(() => {
  ...
  if (params.get('smokeGraphDiffReady') !== '1') {
    params.set('smokeGraphDiffReady', '1');
    params.set('graphDiffKind', 'upgrade');
    params.set('graphDiffQuery', 'lib');
  ...
  const rows = () => Array.from(document.querySelectorAll('[data-graph-diff-row]'));
  ...
  const readDiffParams = () => {
    const current = new URLSearchParams(window.location.search);
    return {
      query: current.get('graphDiffQuery') || '',
      kind: current.get('graphDiffKind') || '',
    };
  };
  ...
  try {
    if (!panel || !search || !kind || !reset || !count) {
      throw new Error('Missing graph diff filter controls');
    }
    ...
    const initialParams = readDiffParams();
    ...
    expect('updated URL query', readDiffParams().query, 'core');
    ...
    const resetParams = readDiffParams();
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_path = write_graph_diff_filter_smoke_page(args.output)
    print(output_path)
    return 0
