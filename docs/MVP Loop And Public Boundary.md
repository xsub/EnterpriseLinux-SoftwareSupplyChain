...
- Generate a self-checking browser smoke HTML page for static graph-diff
  filter deep links, URL updates, and reset behavior.
- Generate a self-checking browser smoke HTML page for static graph diff-tree
  filter deep links, URL updates, and reset behavior.
...
- Render report bundle manifests as static HTML with bundle metadata, member
  report digests, and triage summary artifact rows.
...
- Safely verify deterministic report bundle archives after CI/workbench
  handoff.
...
- Enforce documented `oneOf` schema branches in the dependency-free validator.
...
- Emit global graph-diff top package-change findings in JSON and HTML so broad
...
- Package verified graph export batches as deterministic `.tar.gz` handoff
  artifacts and verify those archives after transfer.
...
python -B -m src.cli submission-plan-index --input tests/fixtures/export-batch-submission-plan.json --input tests/fixtures/report-bundle-submission-plan.json --format text --fail-on-status warn
