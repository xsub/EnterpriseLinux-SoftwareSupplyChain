...
edgp albs-build-timing --build-id 17812 --format text
...
edgp albs-release-completeness --build-id 17812 --build-id 17813 --format text
...
edgp query --source rpm-repo --rpm-repo-source https://repo.almalinux.org/almalinux/10/BaseOS/x86_64/os/ --operation most-depended-upon --format text
...
edgp query-bundle --path package-lock.json --operation reachable --node app --output-dir reports/query --format text --triage-summary
```
...
`diffTreeNetEdgeDelta`, and bundle catalogs preserve those values per bundle,
per source kind, and in their global summary. Text output includes non-zero cone
rollups so CI logs can show drift scale without opening JSON or HTML. They also include
...
edgp benchmark --nodes 1000 --fanout 3 --backend auto --format text
...
generation. `--format text` prints compact counts and first results for CI logs
without replacing the full JSON. `edgp query-bundle` wraps the same traversal
output in an `edgp.query.report.v1` document and renders it as static HTML with
a verifiable manifest; it can also print a one-line bundle summary with
`--format text`. Query selectors accept exact package IDs, such as
...
CSR artifact matrix views, triage status, and top findings when present. The
validation triage panel breaks out failed checks and diff-tree policy failures
instead of hiding them inside a nested summary object.
...
gates, memory-mapped CSR query evidence, or deterministic CSR performance
evidence.
...
parallel-query/performance rollups are emitted as a compact `sourceKinds=...`
token, so CI logs can show which input family carried the drift, CSR query
workload, or benchmark evidence without opening JSON or HTML.
